...
function Vbias(r, slist, w, sigma, domain)
	result = 0.0
	for i in eachindex(slist)
		result += w[i] * exp(-(x(r) - slist[i][1]) ^ 2 / (2 * (sigma * (domain[1][2] - domain[1][1])) ^ 2) - (y(r) - slist[i][2]) ^ 2 / (2 * (sigma * (domain[2][2] - domain[2][1])) ^ 2))
	end
	return result
end

function grad_V(r, slist, w, sigma, domain)
	grad = ForwardDiff.gradient(V, r)
	for i in eachindex(slist)
		Vinc(r) = w[i] * exp(-(x(r) - slist[i][1]) ^ 2 / (2 * (sigma * (domain[1][2] - domain[1][1])) ^ 2) - (y(r) - slist[i][2]) ^ 2 / (2 * (sigma * (domain[2][2] - domain[2][1])) ^ 2))
...
function well_tempered_mtd()
	...
	for i in 1:steps
		...
		if i % stride == 0
			push!(traj, (t, x([x1, x2, x3, x4]), y([x1, x2, x3, x4]), Vbias([x1, x2, x3, x4], slist, w, sigma, domain_cv)))
		end
