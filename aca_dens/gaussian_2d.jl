...
function Vbias(s, rho, basis, domain, kT)
	...
	return kT * log(max(dens_eval(rho, basis, s), 1))
end

function Vtop(rho, basis, domain, samples, kT)
	max = 0.0
	for s in samples
		result = Vbias(s, rho, basis, domain, kT)
		if result > max
			max = result
		end
	end
