...
rhohat(x, y) = pdf(ik, x, y)
domain_cv_small = ((first(kde_result.x), last(kde_result.x)), (first(kde_result.y), last(kde_result.y)))
F2 = ResFunc(rhohat, domain_cv_small)
fp = open("pivots2.txt")
initIJ(F2, eval(Meta.parse(readline(fp))))
close(fp)
...
rhohat(x, y) = pdf(ik, x, y)
domain_cv_small = ((first(kde_result.x), last(kde_result.x)), (first(kde_result.y), last(kde_result.y)))
F3 = ResFunc(rhohat, domain_cv_small)
fp = open("pivots3.txt")
initIJ(F3, eval(Meta.parse(readline(fp))))
close(fp)

data4 = readdlm("colvar_bias4.txt", ' ', Float64)
data = vcat(data, data4)
len4 = length(data4[:, 1])
weights = [weights; [exp(Vbias(F3, data4[i, 2], data4[i, 3])) for i in 1:len4]]
# kde_result = kde(data[:,2:3], weights = weights, npoints = (nbins, nbins))
# kde_result = kde(data[:,2:3], weights = weights, bandwidth = (0.1, 0.1), npoints = (nbins, nbins))
kde_result = kde(data[:,2:3], weights = weights, bandwidth = (0.2, 0.2), npoints = (nbins, nbins))
ik = InterpKDE(kde_result)
rhohat(x, y) = pdf(ik, x, y)
domain_cv_small = ((first(kde_result.x), last(kde_result.x)), (first(kde_result.y), last(kde_result.y)))
F4 = ResFunc(rhohat, domain_cv_small)
fp = open("pivots4.txt")
initIJ(F4, eval(Meta.parse(readline(fp))))
close(fp)

data5 = readdlm("colvar_bias5.txt", ' ', Float64)
data = vcat(data, data5)
len5 = length(data5[:, 1])
weights = [weights; [exp(Vbias(F4, data5[i, 2], data5[i, 3])) for i in 1:len5]]
# kde_result = kde(data[:,2:3], weights = weights, npoints = (nbins, nbins))
# kde_result = kde(data[:,2:3], weights = weights, bandwidth = (0.1, 0.1), npoints = (nbins, nbins))
kde_result = kde(data[:,2:3], weights = weights, bandwidth = (0.2, 0.2), npoints = (nbins, nbins))
ik = InterpKDE(kde_result)
rhohat(x, y) = pdf(ik, x, y)
domain_cv_small = ((first(kde_result.x), last(kde_result.x)), (first(kde_result.y), last(kde_result.y)))
F5 = ResFunc(rhohat, domain_cv_small)
fp = open("pivots5.txt")
initIJ(F5, eval(Meta.parse(readline(fp))))
close(fp)

data6 = readdlm("colvar_bias6.txt", ' ', Float64)
data = vcat(data, data6)
len6 = length(data6[:, 1])
weights = [weights; [exp(Vbias(F5, data6[i, 2], data6[i, 3])) for i in 1:len6]]

println("$(minimum(data6[:,2])) $(maximum(data6[:,2])) $(minimum(data6[:,3])) $(maximum(data6[:,3]))")
kde_result = kde(data6[:,2:3], npoints = (nbins, nbins))
println("$(kde_result.x) $(kde_result.y)")

ik = InterpKDE(kde_result)
rhohat(x, y) = pdf(ik, x, y)
open("kde6.txt", "w") do file
...
x1, x2, x3, x4 = data[length(data[:, 1]), 4:7]
v1 = v2 = v3 = v4 = 0.0
...
open("colvar_bias7.txt", "w") do file
	for step in traj
