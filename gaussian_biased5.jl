...
println("$(minimum(data5[:,2])) $(maximum(data5[:,2])) $(minimum(data5[:,3])) $(maximum(data5[:,3]))")
kde_result = kde(data5[:,2:3], npoints = (nbins, nbins))
println("$(kde_result.x) $(kde_result.y)")

ik = InterpKDE(kde_result)
rhohat(x, y) = pdf(ik, x, y)
open("kde5.txt", "w") do file
...
open("colvar_bias6.txt", "w") do file
	for step in traj
