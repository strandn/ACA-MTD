...
println("$(minimum(data4[:,2])) $(maximum(data4[:,2])) $(minimum(data4[:,3])) $(maximum(data4[:,3]))")
kde_result = kde(data4[:,2:3], npoints = (nbins, nbins))
println("$(kde_result.x) $(kde_result.y)")

ik = InterpKDE(kde_result)
rhohat(x, y) = pdf(ik, x, y)
open("kde4.txt", "w") do file
...
open("colvar_bias5.txt", "w") do file
	for step in traj
